...
dashboardRouter.get("/wins-vs-length", async (req, res, next) => {
    ...
    for (let bucketStart = START; bucketStart < INFINITE_START; bucketStart += BUCKET_SIZE) {
    ...
    where
        ...
        if (Number(bucketData[0].number_of_games) === 0) continue
        buckets.push({
            name: `${bucketStart} - ${bucketStart + BUCKET_SIZE}`,
            winPercentage: Math.floor(bucketData[0].number_of_wins / bucketData[0].number_of_games * 100),
